...
export class SupabaseStore implements Store {
  ...
  async listEntries() {
    const { data, error } = await this.db.from("entries").select("*").order("due_date");
    if (error) fail("listEntries", error);
    return data.map(toEntry);
  }

  async getEntry(id: string) {
    const { data, error } = await this.db.from("entries").select("*").eq("id", id).maybeSingle();
    if (error) fail("getEntry", error);
    return data ? toEntry(data) : null;
  }

  async createEntry(input: Parameters<Store["createEntry"]>[0]) {
    const { data, error } = await this.db.from("entries").insert(input).select("*").single();
    if (error) fail("createEntry", error);
    return toEntry(data);
  }

  async updateEntry(id: string, patch: EntryPatch, guard?: Guard) {
    let q = this.db.from("entries").update(patch).eq("id", id);
  ...
  async deleteEntry(id: string) {
    const { error } = await this.db.from("entries").delete().eq("id", id);
  ...
  async updateSend(id: string, patch: Parameters<Store["updateSend"]>[1]) {
    const { error } = await this.db.from("reminder_log").update(patch).eq("id", id);
